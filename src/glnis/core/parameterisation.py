...
from glnis.utils.types import GraphProperties, LayerData, ParameterisationLayerConfig
...
class Parameterisation(ABC):
    ...
    def _to_layer_input(self, input: LayerData) -> Tuple[NDArray, NDArray]:
        """
    ...
    def _to_layer_output(
        self,
        layer_input: LayerData,
        jac_param: NDArray,
        cont_param: NDArray,
        disc_param: NDArray | None,
    ) -> LayerData:
        """
        ...
        if disc_param is None:
            disc_param = np.zeros((layer_input.n_points, 0), dtype=layer_input.dtype)
    ...
    def _to_generation_lmb(
        self, momenta: NDArray, discrete: NDArray, inverse: bool = False
    ) -> NDArray:
        """
        ...
        result = sample_transform @ momenta.reshape(
            -1, self.graph_properties.n_loops, 3
        )
...
class LayeredParameterisation:
    ...
    def __init__(
        ...
        self.condition_integrand_first = param_configs.get(
            "condition_integrand_first", False
        )
        self.use_f128 = param_configs.get("use_f128", False)
        for i_layer, config in enumerate(
            [ParameterisationLayerConfig.from_dict(config) for config in layer_configs]
        ):
            is_first_layer = (i_layer + 1) == len(layer_configs)
            next_param = None if i_layer == 0 else param_layers[-1]
            config.param_kwargs.update(
                dict(
            ...
            match config.param_type.lower():
                ...
                case "madspace":
                    p = MadspaceParameterisation(**config.param_kwargs)
                case "mc_layer":
                    ...
                    config.param_kwargs.update(
                        dict(
                            is_first_layer=next_param.is_first_layer,
    ...
    def parameterise(
        ...
        n_param_disc = len(self.discrete_dims)
        if n_param_disc == 0:
            param_disc = np.zeros((discrete.shape[0], 0), dtype=discrete.dtype)
            pass_disc_to_integrand = discrete
        elif self.condition_integrand_first:
            param_disc = discrete[..., -n_param_disc:]
            pass_disc_to_integrand = discrete[..., :-n_param_disc]
        else:
            param_disc = discrete[..., :n_param_disc]
            pass_disc_to_integrand = discrete[..., n_param_disc:]
...
class MomtropParameterisation(Parameterisation):
    ...
    def __init__(
        ...
        **kwargs: Dict[str, Any],
        ...
        match self.edge_weight:
            case int() | float():
                self.edge_weight = self.graph_properties.n_edges * [
            ...
            case _:
                default_weight = (
                    (3 * self.graph_properties.n_loops + 3 / 2)
                    / self.graph_properties.n_edges
                    / 2
                )
        ...
        mt_edges = [
            ...
            for src_dst, ismassive, weight in zip(
                self.graph_properties.edge_src_dst_vertices,
        ...
        momentum_shifts = [
            ...
            for shift in self.graph_properties.edge_momentum_shifts
        ]
        ...
        if self.mask_redundant:
            continuous = np.hstack(
                [
                    continuous,
                    np.zeros(
                        (continuous.shape[0], self.graph_properties.n_edges - 1),
    ...
    def _layer_continuous_dim_in(self) -> int:
        if self.mask_redundant:
            return (
                self.momtrop_sampler.get_dimension() - self.graph_properties.n_edges + 1
            )
...
class MadspaceParameterisation(Parameterisation):
...
class MCLayer(Parameterisation, ABC):
    ...
    def __init__(self, param: Parameterisation, **kwargs):
        self.param = param
        ...
        self.channel_masses = np.array(self.graph_properties.edge_masses)[self.lmbs]
    ...
    def _layer_parameterise(
        ...
        jac, momentum, _ = self.param._layer_parameterise(continuous, discrete)
